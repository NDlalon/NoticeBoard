...
async function renderPDFpage(pdfDoc,data){
    ...
    for (let pageNum = 1; pageNum <= pdfDoc.numPages; pageNum++) {
        ...
        // 找 placeholder
        const index = carousel.contentItems.findIndex(
            item => item.dataset.src === data.src && item.dataset.type=== 'placeholder'
        );

        // 替換 placeholder
        if (index !== -1) {
            contentContainer.replaceChild(contentItem, carousel.contentItems[index]);
            carousel.contentItems[index] = contentItem;
        }
